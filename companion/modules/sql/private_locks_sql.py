...
pm_locks = sa.Table("private_locks", metadata,
                    ...
                    sa.Column("forwards", sa.Boolean(), nullable=False),
...
def update_restriction(lock_type, is_locked):
    ...
    if lock_type == "audio":
    ...
    elif lock_type == "image":
        query = sa.update(pm_locks).values(image=locked)
    elif lock_type == "gif":
        ...
        query = sa.update(pm_locks).values(forwards=locked)
    elif lock_type == 'game':
